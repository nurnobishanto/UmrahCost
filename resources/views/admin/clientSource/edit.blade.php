    ...
    <div class="ams-panel-wpr panel-md">
        <div class="ams-panel">
            <div class="panel-heading">
                <h5 class="panel-title">Edit Client Source</h5>
                <div>
                    @if (check_permission('Client Source List'))
                        <a href="{{ route('admin.clientSource.index') }}" class="btn add-btn"><i class="fas fa-list-ul"></i> Client Source List</a>
                    @endif
                </div>
            </div>
            <div class="panel-body">
                <div class="ams-customer-add-form">
                    <form method="POST" action="{{ route('admin.clientSource.update', $clientSource->id) }}" enctype="multipart/form-data" class="ams-form">
                        @csrf
                        @method('PUT')
                        <fieldset class="ams-input input-verticle">
                            <label for="name">Name<sup class="required">*</sup></label>
                            <input type="text" value="{{ old('name',$clientSource->name) }}" name="name" id="name" placeholder="Enter Name">
                            @error('name')
