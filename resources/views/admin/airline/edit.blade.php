    ...
    <div class="ams-panel-wpr panel-md">
        <div class="ams-panel">
            <div class="panel-heading">
                <h5 class="panel-title">Edit Airline</h5>
                <div>
            ...
            <div class="panel-body">
                <div class="ams-customer-add-form">
                    <form method="POST" action="{{ route('admin.airline.update', $airline->id) }}" enctype="multipart/form-data" class="ams-form">
                        @csrf
                        @method('PUT')
                        <fieldset class="ams-input input-verticle">
                            <label for="name">Name<sup class="required">*</sup></label>
                            <input type="text" value="{{ old('name', $airline->name) }}" name="name" id="name" placeholder="Enter Name">
                            @error('name')
                        ...
                        <fieldset class="ams-input input-verticle">
                            ...
                            <select name="package" id="package" class="form-control">
                                ...
                                @foreach ($packages as $package)
                                    <option @if($package->id == old('package', $airline->package_id)) selected @endif value="{{ $package->id }}">{{ $package->name }}</option>
                                @endforeach
                            </select>
                            @error('package')
                                <span class="text-danger">{{ $message }}</span>
                        ...
                        <fieldset class="ams-input input-verticle">
                            ...
                            <input type="number" step="any" value="{{ old('cost',$airline->cost) }}" name="cost" id="cost" placeholder="Enter Cost of Visa">
                            @error('cost')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </fieldset>
