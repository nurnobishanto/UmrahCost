    ...
    <div class="ams-panel-wpr panel-md">
        <div class="ams-panel">
            <div class="panel-heading">
                <h5 class="panel-title">Edit Status</h5>
                <div>
            ...
            <div class="panel-body">
                <div class="ams-customer-add-form">
                    <form method="POST" action="{{ route('admin.status.update', $status->id) }}" enctype="multipart/form-data" class="ams-form">
                        @csrf
                        @method('PUT')
                        <fieldset class="ams-input input-verticle">
                            <label for="name">Name<sup class="required">*</sup></label>
                            <input type="text" value="{{ old('name',$status->name) }}" name="name" id="name" placeholder="Enter Name">
                            @error('name')
                        ...
                        <fieldset class="ams-input input-verticle">
                            ...
                            <input type="text" value="{{ old('color',$status->color) }}" name="color" id="color" placeholder="Enter Color">
                            @error('color')
                                <span class="text-danger">{{ $message }}</span>
                            @enderror
                        </fieldset>
