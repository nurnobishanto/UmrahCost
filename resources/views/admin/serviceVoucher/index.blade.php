    ...
    <div class="ams-panel-wpr">
        <div class="ams-panel">
            <div class="panel-heading">
                <h5 class="panel-title"> Service Voucher List</h5>
                @if (check_permission('Service Voucher Create'))
                    <a href="{{ route('admin.serviceVoucher.create') }}" class="panel-item">+ Add New Voucher</a>          
                @endif
            </div>
            <div class="panel-body">
                <form id="table-search-form" action="{{ route('admin.serviceVoucher.index') }}" method="GET"
                        ...
                        <div class="panel-search-box">
                            <div>
                                ...
                                {{-- <fieldset class="ams-input input-verticle">
                                    ...
                                    <select class="unit-select-box form-select" name="package_id" id="package_id">
                                        <option value="">All</option>
                    ...
                    <table id="supplierList" class="table table-bordered table-responsive" style="width: 100%">
                        ...
                        <tbody>
                            @foreach ($serviceVouchers as $serviceVoucher)
                                <tr>
                                    <td>{{ $serviceVouchers->firstItem() + $loop->index }}</td>
                                    <td>{{ $serviceVoucher?->client?->name }}</td>
                                    <td>{{ $serviceVoucher?->voucher_guests_count }}</td>
                                    <td>{{ common_date_time_format($serviceVoucher?->created_at) }}</td>
                                    {{-- <td>{{ $serviceVoucher?->mailSentBy?->name }}</td> --}}
                                    <td>
                                        <div class="edit-icons">
                                            <div class="action-buttons">
                                                ...
                                                <a target="_blank"
                                                    href="{{ route('invoice.serviceVoucher', encrypt($serviceVoucher->id)) }}">
                                                    <i class="fa fa-list bg-info"></i>
                                                </a>
                                                <a href="{{ route('admin.serviceVoucher.edit', $serviceVoucher->id) }}">
                                                    <i class="fa fa-edit bg-success"></i>
                                                </a>
                                                    ...
                                                    <div class="dropdown-menu" x-placement="bottom-start"
                                                        style="position: absolute; transform: translate3d(0px, 35px, 0px); top: 0px; left: 0px; will-change: transform;">       
                                                            @foreach ($statuses as $status)
                                                                <a href="@if($status->id == $serviceVoucher->status_id) # @else {{ route('admin.serviceVoucher.changeStatus',[$serviceVoucher->id,$status->id]) }} @endif" class="dropdown-item">{{ $status->name }}</a>
                                                            @endforeach
                                                    </div>
                                                </div> --}}
                                            </div>
                ...
                <div class="dataTable-info-wpr">
                    <div class="dataTable-info" id="supplierList-info">
                        Showing {{ $serviceVouchers->firstItem() }} to {{ $serviceVouchers->lastItem() }} of
                        {{ $serviceVouchers->total() }}
                        entries
                    </div>
                    <div class="dataTable-pagination" id="supplierList-pagination">
                        {{ $serviceVouchers->appends(Request::all())->links('pagination::bootstrap-4') }}
                    </div>
                </div>
            </div>
        </div>
...
@push('script')
    <script>
        ...
        function exportData(type) {
            ...
            let ExportUrl = '{{ route('admin.serviceVoucher.index', '#') }}';
            EL_form.action = ExportUrl.replace('#', type);
            EL_form.submit();
            EL_form.action = '{{ route('admin.serviceVoucher.index') }}';
            $('#table_search').focus();
        }
    </script>
@endpush
