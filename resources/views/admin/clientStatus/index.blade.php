...
@endpush
@section('content')
    <!-- Client Status Datatable -->
    <div class="ams-panel-wpr">
        <div class="ams-panel">
            <div class="panel-heading">
                <h5 class="panel-title">Client Status List</h5>
                @if (check_permission('Client Status Create'))
                    <a href="{{ route('admin.clientStatus.create') }}" class="panel-item">+ Add Client Status</a>
                @endif
            </div>
    ...
    <script>
        $(function() {
            $('#datatable').DataTable({
                ...
                "pageLength": 10,
                ajax: '{!! route('admin.clientStatus.index') !!}',
                columns: [{
                        data: 'DT_RowIndex',
                        name: 'DT_RowIndex',
                        orderable: false,
